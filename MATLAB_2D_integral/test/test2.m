...
% przy obliczaniu całki podwójnej wielomianu 2 argumentowego 1 stopnia
...
DispWithPause(sprintf('-> liczenie całki z f(x, y) = %fx + %fy + %f' ...
    , p(3), p(2), p(1)));
DispWithPause(sprintf('na obszarze [%f, %f] x [%f, %f]', a, b, c, d));
DispWithPause(repmat('-', 1, rowLength));

% funckja
f = @(x, y) p(3)*x + p(2)*y + p(1);

% wynik analityczny
wyn = p(3)*integral_xnym(1 , 0, a, b, c, d) + ...
