...
% przy obliczaniu całki podwójnej wielomianu 2 argumentowego 2 stopnia
...
DispWithPause(sprintf(['-> liczenie całki z f(x, y) = %fx^2 + %fxy + ' ...
    '%fy^2 + %fx + %fy + %f'], p(6), p(5), p(4), p(3), p(2), p(1)));
DispWithPause(sprintf('na obszarze [%f, %f] x [%f, %f]', a, b, c, d));
DispWithPause(repmat('-', 1, rowLength));

f = @(x, y) p(6)*x^2 + p(5)*x*y + p(4)*y^2 + p(3)*x + p(2)*y + p(1);

wyn = p(6)*integral_xnym(2, 0, a, b, c, d) + ...
