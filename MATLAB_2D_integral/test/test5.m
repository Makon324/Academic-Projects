...
% przy obliczaniu całki podwójnej wielomianu 2 argumentowego 4 stopnia
...
test_desc_path = 'desc_test5.txt'; % ścieżka do pliku z opisem testu
...
DispWithPause(sprintf(['-> liczenie całki z f(x, y) = %fx^4 + %fx^3y +' ...
    ' %fx^2y^2 + %fxy^3 + %fy^4 + %fx^3 + %fx^2y + %fxy^2 + %fy^3 + ' ...
    '%fx^2 + %fxy + %fy^2 + %fx + %fy + %f'], p(15), p(14), p(13), ...
...
% funckja
f = @(x, y) p(15)*x^4 + p(14)*x^3*y +p(13)*x^2*y^2 + p(12)*x*y^3 + ...
p(11)*y^4 + p(10)*x^3 + p(9)*x^2*y + p(8)*x*y^2 + p(7)*y^3 + p(6)*x^2 + ...
p(5)*x*y + p(4)*y^2 + p(3)*x + p(2)*y + p(1);

% wynik analityczny
wyn = p(15)*integral_xnym(4, 0, a, b, c, d) + ...
